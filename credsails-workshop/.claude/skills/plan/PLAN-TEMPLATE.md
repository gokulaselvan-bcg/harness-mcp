...
**Task ID:** PROJ-123 (or $001 for ad-hoc)
**Created:** YYYY-MM-DD
...
- Architectural constraints (ADRs under `artifacts/architecture/` if present)
- Parallel work opportunities (if any)

## 1b. Related Documentation (Optional)

- Spec: `artifacts/tickets/[TICKET-ID or $001]/spec.md`
- ADRs: `artifacts/architecture/` (link the relevant ADR filenames if any)
...
- [ ] **P001-STEP-NAME**: Step description
  ...
  - **Code quality**:
    - **Follows**: Y### (general) and language-specific IDs (PY###/JY###) as applicable
      - See `.claude/context/coding-standards.md` (entrypoint to full catalogs)
    - **Avoids**: Z### (general) and language-specific IDs (PZ###/JZ###) as applicable
      - See `.claude/context/coding-standards.md` (entrypoint to full catalogs)
...
- Every P### step must reference spec IDs (F/C/M/E/U/I) and cite Y/Z IDs for code quality expectations
